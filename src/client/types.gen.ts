 ...
 * Acquisition
 *
 * How someone arrived, as their browser reported it on first landing.
 *
 * Every field is optional because most visits carry none of them: a person
 * typing the address in has no campaign parameters and no referrer. Bounded
 * lengths because these come from a URL a stranger controls and land in a
 * jsonb column.
 */
...
export type AdminAttemptResultRow = {
     ...
     * Targetuniversities
     */
    targetUniversities?: Array<string> | null;
    /**
     * Country
     */
    country?: string | null;
...
export type AdminQuestionReport = {
     ...
     * Category
     */
    category: 'typo' | 'question_error' | 'wrong_answer' | 'solution_error' | 'diagram' | 'other';
    /**
     * Message
     */
    message?: string | null;
 ...
 * AnalyticsEvent
 *
 * One recorded event. user_id is deliberately absent — it is taken from
 * the bearer token server-side, so a client cannot write events as somebody
 * else.
 */
export type AnalyticsEvent = {
     ...
     * Eventname
     */
    eventName: 'diagnostic_started' | 'question_answered' | 'diagnostic_submitted' | 'diagnostic_timed_out' | 'retake_started' | 'second_subject_started' | 'report_viewed' | 'diagnostic_cta_clicked' | 'dashboard_viewed' | 'report_reopened' | 'resume_clicked' | 'retake_clicked' | 'review_opened' | 'solution_viewed' | 'incorrect_filter_used';
    /**
     * Attemptid
     */
    attemptId?: string | null;
 ...
 *
 * What the frontend needs to decide between "buy", "upgrade" and "you
 * already own this".
 *
 * Also what the post-checkout return page polls: the pass is granted by the
 * webhook, not by the redirect, so there is a short window after paying in
 * which `has_pass` is still false.
 */
export type BillingStatusResponse = {
     ...
     * Coveredtests
     */
    coveredTests?: Array<string>;
    /**
     * Seasons
     */
    seasons?: Array<SeasonOffer>;
};
 ...
 * BreakStateResponse
 *
 * Returned by the two rest-break endpoints. The deadline comes back with
 * it because ending a break moves it, and the screen must re-read the clock
 * from the server rather than adding the pause up itself.
 */
 ...
 * CheckoutRequest
 *
 * Which season to buy, and where to send the student back to if they
 * abandon Stripe's page.
 */
 ...
 * CheckoutResponse
 *
 * Stripe's hosted-checkout URL. The browser navigates to it; we never
 * render a card form ourselves, which is what keeps this integration out of
 * PCI scope.
 */
...
/**
 * DeclareAccessArrangementsBody
 *
 * A student saying what arrangements they have for the real test.
 *
 * confirmed is the declaration itself, and is required whenever they claim
 * anything: 'I have been granted, or have applied for, this arrangement'.
 * Saying they have none needs no confirmation.
 */
export type DeclareAccessArrangementsBody = {
    /**
     * Extratimepercent
     */
...
export type DiagnosticSetResponse = {
     ...
     */
    questionsWithSolutions?: number | null;
    /**
     * Status
     */
    status: 'draft' | 'published';
 ...
 *
 * What this student has already told us, so the survey does not ask
 * again. Only country is read by the survey today: sitting and universities
 * are worth re-asking, since both change over a season, and a country does
 * not.
 */
export type DiagnosticSurveyAnswers = {
    /**
     * Testsitting
     */
    testSitting?: 'october_2026' | 'january_2027' | 'undecided' | null;
    /**
     * Targetuniversities
     */
    targetUniversities?: Array<string> | null;
...
/**
 * DiagnosticSurveyBody
 *
 * The two-question survey shown once, after a first submission.
 *
 * Both fields optional and independently so: the survey is skippable, and
 * skippable has to mean a student can answer one question and not the other
 * without the form refusing them.
 */
export type DiagnosticSurveyBody = {
    /**
     * Testsitting
     */
    testSitting?: 'october_2026' | 'january_2027' | 'undecided' | null;
...
/**
 * DiagnosticSurveyResponse
 */
export type DiagnosticSurveyResponse = {
    /**
     * Message
     */
    message: string;
 ...
 *
 * Says the same thing for every address, always — see the endpoint.
 */
 ...
 * EmailCodeSignInBody
 */
export type EmailCodeSignInBody = {
    /**
     * Email
     */
    email: string;
 ...
 * GoogleSignInBody
 *
 * The ID token Google hands the browser. Not a password and not a
 * secret of ours — it is signed by Google and verified server-side.
 */
 ...
 * HeldPass
 *
 * One entitlement row, as an admin reads it.
 */
...
/**
 * IngestAnalyticsEventsBody
 */
export type IngestAnalyticsEventsBody = {
    /**
     * Events
     */
 ...
 * MicrosoftSignInBody
 *
 * The ID token Microsoft hands the browser. Not a password and not a
 * secret of ours — it is signed by Microsoft and verified server-side.
 *
 * Carries the nonce the browser generated for this sign-in so the backend can
 * check the token was minted for this attempt; Google's flow has no
 * equivalent because its library handles the binding itself.
 */
export type MicrosoftSignInBody = {
    /**
     * Credential
     */
    credential: string;
...
/**
 * MyAccessArrangements
 *
 * The student's own view of their arrangements.
 *
 * answered is whether they have been asked yet: a row exists once they have
 * said yes or no, or once an admin has set one for them. Until then the
 * instructions screen asks before letting them start.
 */
export type MyAccessArrangements = {
    /**
     * Answered
     */
    answered?: boolean;
 ...
 * PaperProgress
 *
 * One paper in a pass holder's tests, and where they are with it.
 *
 * Every paper is listed, sat or not — "which ones have they not touched" is
 * half of what this view is for, and a list of attempts cannot answer it.
 */
export type PaperProgress = {
     ...
     * Lateststatus
     */
    latestStatus?: 'in_progress' | 'submitted' | 'timed_out' | 'abandoned' | null;
    /**
     * Latestscore
     */
 ...
 * QuestionReportBody
 */
export type QuestionReportBody = {
    /**
     * Category
     */
    category: 'typo' | 'question_error' | 'wrong_answer' | 'solution_error' | 'diagram' | 'other';
 ...
 *
 * Deliberately says nothing about the question. Mid-exam, anything more
 * than an acknowledgement could leak whether the report was right.
 */
 ...
 */
export type QuestionReportsResponse = {
    /**
     * Questions
     */
    questions: Array<ReportedQuestion>;
};
 ...
 * ReportedQuestion
 *
 * One question with every report about it, which is how an admin reads
 * them: three reports on one question are one problem, not three.
 */
export type ReportedQuestion = {
     ...
     * Reports
     */
    reports: Array<AdminQuestionReport>;
};

/**
 * RescoreApplyResponse
 */
export type RescoreApplyResponse = {
    /**
     * Rescored
     */
    rescored: number;
     ...
     * Results
     */
    results: Array<RescoreResult>;
};

/**
 * RescorePreviewResponse
 */
export type RescorePreviewResponse = {
    /**
     * Questionid
     */
    questionId: string;
 ...
 * RescoreRow
 *
 * One past sitting whose mark on this question would change.
 */
 ...
 * ReviewOption
 *
 * One option as it appears in review, after the attempt is over.
 *
 * Unlike the in-exam shape, this one carries is_correct: the whole point of
 * review is seeing which was right. `misconception` is present only on the
 * option the student actually chose, and only when it was wrong — the other
 * distractors' notes describe mistakes they did not make, and showing all of
 * them turns a specific piece of feedback into a wall of text.
 */
 ...
 * SeasonOffer
 *
 * One buyable season, as the unlock UI needs it.
 */
...
export type SeasonPassHolder = {
     ...
     * Passes
     */
    passes: Array<HeldPass>;
    /**
     * Papers
     */
    papers: Array<PaperProgress>;
    /**
     * Paperscompleted
     */
...
/**
 * SeasonPassesResponse
 */
export type SeasonPassesResponse = {
    /**
     * Holders
     */
    holders: Array<SeasonPassHolder>;
};
...
/**
 * StudentAccommodationBody
 */
export type StudentAccommodationBody = {
    /**
     * Extratimepercent
     */
    extraTimePercent?: number;
...
/**
 * StudentAccommodationResponse
 */
export type StudentAccommodationResponse = {
    /**
     * Studentid
     */
    studentId: string;
 ...
 */
export type StudentAccommodationsResponse = {
    /**
     * Accommodations
     */
    accommodations: Array<StudentAccommodationResponse>;
};

/**
 * StudentAttemptRow
 *
 * One of the signed-in student's own attempts, for their results page.
 *
 * Deliberately not AdminAttemptResultRow: that row carries the student's
 * email (pointless when you are the student) and engaged time (an admin
 * diagnostic), and its answered_count is derived from is_correct, which is
 * only written at finalisation and therefore reads zero for an attempt still
 * in progress. This page's whole left-hand column is in-progress attempts, so
 * it counts selected_option instead.
 */
 ...
 */
export type StudentAttemptsResponse = {
    /**
     * Attempts
     */
    attempts: Array<StudentAttemptRow>;
};
...
/**
 * UpdateQuestionReportBody
 */
export type UpdateQuestionReportBody = {
    /**
     * Status
     */
    status: 'open' | 'fixed' | 'dismissed';
...
export type DiagnosticSurveyAnswersUsersDiagnosticSurveyGetData = {
    body?: never;
    path?: never;
    query?: never;
...
export type DiagnosticSurveyAnswersUsersDiagnosticSurveyGetResponses = {
    /**
     * Successful Response
     */
    200: DiagnosticSurveyAnswers;
};

export type DiagnosticSurveyAnswersUsersDiagnosticSurveyGetResponse = DiagnosticSurveyAnswersUsersDiagnosticSurveyGetResponses[keyof DiagnosticSurveyAnswersUsersDiagnosticSurveyGetResponses];

export type DiagnosticSurveyUsersDiagnosticSurveyPostData = {
    body: DiagnosticSurveyBody;
    path?: never;
    query?: never;
    url: '/users/diagnostic-survey';
};

export type DiagnosticSurveyUsersDiagnosticSurveyPostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type DiagnosticSurveyUsersDiagnosticSurveyPostError = DiagnosticSurveyUsersDiagnosticSurveyPostErrors[keyof DiagnosticSurveyUsersDiagnosticSurveyPostErrors];

export type DiagnosticSurveyUsersDiagnosticSurveyPostResponses = {
    /**
     * Successful Response
     */
    200: DiagnosticSurveyResponse;
};

export type DiagnosticSurveyUsersDiagnosticSurveyPostResponse = DiagnosticSurveyUsersDiagnosticSurveyPostResponses[keyof DiagnosticSurveyUsersDiagnosticSurveyPostResponses];

export type MyAccessArrangementsUsersAccessArrangementsGetData = {
    body?: never;
    path?: never;
    query?: never;
...
export type MyAccessArrangementsUsersAccessArrangementsGetResponses = {
    /**
     * Successful Response
     */
    200: MyAccessArrangements;
};

export type MyAccessArrangementsUsersAccessArrangementsGetResponse = MyAccessArrangementsUsersAccessArrangementsGetResponses[keyof MyAccessArrangementsUsersAccessArrangementsGetResponses];

export type DeclareAccessArrangementsUsersAccessArrangementsPutData = {
    body: DeclareAccessArrangementsBody;
    path?: never;
    query?: never;
    url: '/users/access-arrangements';
};

export type DeclareAccessArrangementsUsersAccessArrangementsPutErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type DeclareAccessArrangementsUsersAccessArrangementsPutError = DeclareAccessArrangementsUsersAccessArrangementsPutErrors[keyof DeclareAccessArrangementsUsersAccessArrangementsPutErrors];

export type DeclareAccessArrangementsUsersAccessArrangementsPutResponses = {
    /**
     * Successful Response
     */
    200: MyAccessArrangements;
...
export type DeclareAccessArrangementsUsersAccessArrangementsPutResponse = DeclareAccessArrangementsUsersAccessArrangementsPutResponses[keyof DeclareAccessArrangementsUsersAccessArrangementsPutResponses];
...
export type RequestEmailLoginCodeUsersEmailCodePostData = {
    body: EmailCodeRequestBody;
    path?: never;
    query?: never;
    url: '/users/email-code';
};

export type RequestEmailLoginCodeUsersEmailCodePostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type RequestEmailLoginCodeUsersEmailCodePostError = RequestEmailLoginCodeUsersEmailCodePostErrors[keyof RequestEmailLoginCodeUsersEmailCodePostErrors];

export type RequestEmailLoginCodeUsersEmailCodePostResponses = {
    /**
     * Successful Response
     */
    200: EmailCodeRequestResponse;
};

export type RequestEmailLoginCodeUsersEmailCodePostResponse = RequestEmailLoginCodeUsersEmailCodePostResponses[keyof RequestEmailLoginCodeUsersEmailCodePostResponses];

export type SignInWithEmailCodeUsersEmailCodeVerifyPostData = {
    body: EmailCodeSignInBody;
    path?: never;
    query?: never;
    url: '/users/email-code/verify';
};

export type SignInWithEmailCodeUsersEmailCodeVerifyPostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type SignInWithEmailCodeUsersEmailCodeVerifyPostError = SignInWithEmailCodeUsersEmailCodeVerifyPostErrors[keyof SignInWithEmailCodeUsersEmailCodeVerifyPostErrors];

export type SignInWithEmailCodeUsersEmailCodeVerifyPostResponses = {
    /**
     * Successful Response
     */
    200: UserLoginResponse;
...
export type SignInWithEmailCodeUsersEmailCodeVerifyPostResponse = SignInWithEmailCodeUsersEmailCodeVerifyPostResponses[keyof SignInWithEmailCodeUsersEmailCodeVerifyPostResponses];

export type SignInWithGoogleUsersGooglePostData = {
...
export type SignInWithGoogleUsersGooglePostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type SignInWithGoogleUsersGooglePostError = SignInWithGoogleUsersGooglePostErrors[keyof SignInWithGoogleUsersGooglePostErrors];

export type SignInWithGoogleUsersGooglePostResponses = {
    /**
     * Successful Response
     */
    200: UserLoginResponse;
...
export type SignInWithGoogleUsersGooglePostResponse = SignInWithGoogleUsersGooglePostResponses[keyof SignInWithGoogleUsersGooglePostResponses];

export type SignInWithMicrosoftUsersMicrosoftPostData = {
    body: MicrosoftSignInBody;
    path?: never;
    query?: never;
    url: '/users/microsoft';
};

export type SignInWithMicrosoftUsersMicrosoftPostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type SignInWithMicrosoftUsersMicrosoftPostError = SignInWithMicrosoftUsersMicrosoftPostErrors[keyof SignInWithMicrosoftUsersMicrosoftPostErrors];

export type SignInWithMicrosoftUsersMicrosoftPostResponses = {
    /**
     * Successful Response
     */
    200: UserLoginResponse;
...
export type SignInWithMicrosoftUsersMicrosoftPostResponse = SignInWithMicrosoftUsersMicrosoftPostResponses[keyof SignInWithMicrosoftUsersMicrosoftPostResponses];
...
export type ListDiagnosticQuestionsDiagnosticQuestionsGetData = {
    ...
    query?: {
         ...
         * Ids
         *
         * Return only these questions. Repeat the parameter per id. Meant for a caller that already knows which questions it wants — a set's review screen asking for its own 27 rather than the whole bank.
         */
...
export type ListMyAttemptsDiagnosticAttemptsGetData = {
    body?: never;
    path?: never;
    query?: never;
...
export type ListMyAttemptsDiagnosticAttemptsGetResponses = {
    /**
     * Successful Response
     */
    200: StudentAttemptsResponse;
};

export type ListMyAttemptsDiagnosticAttemptsGetResponse = ListMyAttemptsDiagnosticAttemptsGetResponses[keyof ListMyAttemptsDiagnosticAttemptsGetResponses];
...
export type EndRestBreakDiagnosticAttemptsAttemptIdBreaksDeleteData = {
    body?: never;
    path: {
        /**
         * Attempt Id
...
export type EndRestBreakDiagnosticAttemptsAttemptIdBreaksDeleteErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type EndRestBreakDiagnosticAttemptsAttemptIdBreaksDeleteError = EndRestBreakDiagnosticAttemptsAttemptIdBreaksDeleteErrors[keyof EndRestBreakDiagnosticAttemptsAttemptIdBreaksDeleteErrors];

export type EndRestBreakDiagnosticAttemptsAttemptIdBreaksDeleteResponses = {
    /**
     * Successful Response
     */
    200: BreakStateResponse;
};

export type EndRestBreakDiagnosticAttemptsAttemptIdBreaksDeleteResponse = EndRestBreakDiagnosticAttemptsAttemptIdBreaksDeleteResponses[keyof EndRestBreakDiagnosticAttemptsAttemptIdBreaksDeleteResponses];

export type StartRestBreakDiagnosticAttemptsAttemptIdBreaksPostData = {
    body?: never;
    path: {
        /**
         * Attempt Id
...
export type StartRestBreakDiagnosticAttemptsAttemptIdBreaksPostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type StartRestBreakDiagnosticAttemptsAttemptIdBreaksPostError = StartRestBreakDiagnosticAttemptsAttemptIdBreaksPostErrors[keyof StartRestBreakDiagnosticAttemptsAttemptIdBreaksPostErrors];

export type StartRestBreakDiagnosticAttemptsAttemptIdBreaksPostResponses = {
    /**
     * Successful Response
     */
    200: BreakStateResponse;
...
export type StartRestBreakDiagnosticAttemptsAttemptIdBreaksPostResponse = StartRestBreakDiagnosticAttemptsAttemptIdBreaksPostResponses[keyof StartRestBreakDiagnosticAttemptsAttemptIdBreaksPostResponses];
...
export type GetAttemptReviewDiagnosticAttemptsAttemptIdReviewGetData = {
    body?: never;
    path: {
        /**
         * Attempt Id
...
export type GetAttemptReviewDiagnosticAttemptsAttemptIdReviewGetErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type GetAttemptReviewDiagnosticAttemptsAttemptIdReviewGetError = GetAttemptReviewDiagnosticAttemptsAttemptIdReviewGetErrors[keyof GetAttemptReviewDiagnosticAttemptsAttemptIdReviewGetErrors];

export type GetAttemptReviewDiagnosticAttemptsAttemptIdReviewGetResponses = {
    /**
     * Successful Response
     */
...
export type GetAttemptReviewDiagnosticAttemptsAttemptIdReviewGetResponse = GetAttemptReviewDiagnosticAttemptsAttemptIdReviewGetResponses[keyof GetAttemptReviewDiagnosticAttemptsAttemptIdReviewGetResponses];
...
export type PreviewRescoreDiagnosticAdminQuestionsQuestionIdRescoreGetData = {
    body?: never;
    path: {
        /**
         * Question Id
...
export type PreviewRescoreDiagnosticAdminQuestionsQuestionIdRescoreGetErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type PreviewRescoreDiagnosticAdminQuestionsQuestionIdRescoreGetError = PreviewRescoreDiagnosticAdminQuestionsQuestionIdRescoreGetErrors[keyof PreviewRescoreDiagnosticAdminQuestionsQuestionIdRescoreGetErrors];

export type PreviewRescoreDiagnosticAdminQuestionsQuestionIdRescoreGetResponses = {
    /**
     * Successful Response
     */
    200: RescorePreviewResponse;
};

export type PreviewRescoreDiagnosticAdminQuestionsQuestionIdRescoreGetResponse = PreviewRescoreDiagnosticAdminQuestionsQuestionIdRescoreGetResponses[keyof PreviewRescoreDiagnosticAdminQuestionsQuestionIdRescoreGetResponses];

export type ApplyRescoreDiagnosticAdminQuestionsQuestionIdRescorePostData = {
    body?: never;
    path: {
        /**
         * Question Id
...
export type ApplyRescoreDiagnosticAdminQuestionsQuestionIdRescorePostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type ApplyRescoreDiagnosticAdminQuestionsQuestionIdRescorePostError = ApplyRescoreDiagnosticAdminQuestionsQuestionIdRescorePostErrors[keyof ApplyRescoreDiagnosticAdminQuestionsQuestionIdRescorePostErrors];

export type ApplyRescoreDiagnosticAdminQuestionsQuestionIdRescorePostResponses = {
    /**
     * Successful Response
     */
    200: RescoreApplyResponse;
};

export type ApplyRescoreDiagnosticAdminQuestionsQuestionIdRescorePostResponse = ApplyRescoreDiagnosticAdminQuestionsQuestionIdRescorePostResponses[keyof ApplyRescoreDiagnosticAdminQuestionsQuestionIdRescorePostResponses];

export type ListAccommodationsDiagnosticAdminAccommodationsGetData = {
    body?: never;
    path?: never;
    query?: never;
...
export type ListAccommodationsDiagnosticAdminAccommodationsGetResponses = {
    /**
     * Successful Response
     */
    200: StudentAccommodationsResponse;
};

export type ListAccommodationsDiagnosticAdminAccommodationsGetResponse = ListAccommodationsDiagnosticAdminAccommodationsGetResponses[keyof ListAccommodationsDiagnosticAdminAccommodationsGetResponses];

export type GetAccommodationDiagnosticAdminAccommodationsStudentIdGetData = {
    body?: never;
    path: {
        /**
         * Student Id
...
export type GetAccommodationDiagnosticAdminAccommodationsStudentIdGetErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type GetAccommodationDiagnosticAdminAccommodationsStudentIdGetError = GetAccommodationDiagnosticAdminAccommodationsStudentIdGetErrors[keyof GetAccommodationDiagnosticAdminAccommodationsStudentIdGetErrors];

export type GetAccommodationDiagnosticAdminAccommodationsStudentIdGetResponses = {
    /**
     * Successful Response
     */
    200: StudentAccommodationResponse;
};

export type GetAccommodationDiagnosticAdminAccommodationsStudentIdGetResponse = GetAccommodationDiagnosticAdminAccommodationsStudentIdGetResponses[keyof GetAccommodationDiagnosticAdminAccommodationsStudentIdGetResponses];

export type SetAccommodationDiagnosticAdminAccommodationsStudentIdPutData = {
    body: StudentAccommodationBody;
    path: {
        /**
         * Student Id
         */
...
export type SetAccommodationDiagnosticAdminAccommodationsStudentIdPutErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type SetAccommodationDiagnosticAdminAccommodationsStudentIdPutError = SetAccommodationDiagnosticAdminAccommodationsStudentIdPutErrors[keyof SetAccommodationDiagnosticAdminAccommodationsStudentIdPutErrors];

export type SetAccommodationDiagnosticAdminAccommodationsStudentIdPutResponses = {
    /**
     * Successful Response
     */
    200: StudentAccommodationResponse;
...
export type SetAccommodationDiagnosticAdminAccommodationsStudentIdPutResponse = SetAccommodationDiagnosticAdminAccommodationsStudentIdPutResponses[keyof SetAccommodationDiagnosticAdminAccommodationsStudentIdPutResponses];
...
export type ListSeasonPassHoldersAdminSeasonPassesGetData = {
    body?: never;
    path?: never;
    query?: never;
...
export type ListSeasonPassHoldersAdminSeasonPassesGetResponses = {
    /**
     * Successful Response
     */
    200: SeasonPassesResponse;
};

export type ListSeasonPassHoldersAdminSeasonPassesGetResponse = ListSeasonPassHoldersAdminSeasonPassesGetResponses[keyof ListSeasonPassHoldersAdminSeasonPassesGetResponses];

export type ReportQuestionDiagnosticAttemptsAttemptIdQuestionsQuestionIdReportPostData = {
...
export type ReportQuestionDiagnosticAttemptsAttemptIdQuestionsQuestionIdReportPostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type ReportQuestionDiagnosticAttemptsAttemptIdQuestionsQuestionIdReportPostError = ReportQuestionDiagnosticAttemptsAttemptIdQuestionsQuestionIdReportPostErrors[keyof ReportQuestionDiagnosticAttemptsAttemptIdQuestionsQuestionIdReportPostErrors];

export type ReportQuestionDiagnosticAttemptsAttemptIdQuestionsQuestionIdReportPostResponses = {
    /**
     * Successful Response
     */
    200: QuestionReportResponse;
};

export type ReportQuestionDiagnosticAttemptsAttemptIdQuestionsQuestionIdReportPostResponse = ReportQuestionDiagnosticAttemptsAttemptIdQuestionsQuestionIdReportPostResponses[keyof ReportQuestionDiagnosticAttemptsAttemptIdQuestionsQuestionIdReportPostResponses];

export type ListQuestionReportsAdminQuestionReportsGetData = {
    body?: never;
    path?: never;
    query?: {
        /**
...
export type ListQuestionReportsAdminQuestionReportsGetErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type ListQuestionReportsAdminQuestionReportsGetError = ListQuestionReportsAdminQuestionReportsGetErrors[keyof ListQuestionReportsAdminQuestionReportsGetErrors];

export type ListQuestionReportsAdminQuestionReportsGetResponses = {
    /**
     * Successful Response
     */
    200: QuestionReportsResponse;
};

export type ListQuestionReportsAdminQuestionReportsGetResponse = ListQuestionReportsAdminQuestionReportsGetResponses[keyof ListQuestionReportsAdminQuestionReportsGetResponses];

export type UpdateQuestionReportAdminQuestionReportsReportIdPatchData = {
    body: UpdateQuestionReportBody;
    path: {
        /**
         * Report Id
         */
...
export type UpdateQuestionReportAdminQuestionReportsReportIdPatchErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type UpdateQuestionReportAdminQuestionReportsReportIdPatchError = UpdateQuestionReportAdminQuestionReportsReportIdPatchErrors[keyof UpdateQuestionReportAdminQuestionReportsReportIdPatchErrors];

export type UpdateQuestionReportAdminQuestionReportsReportIdPatchResponses = {
    /**
     * Successful Response
     */
    200: AdminQuestionReport;
};

export type UpdateQuestionReportAdminQuestionReportsReportIdPatchResponse = UpdateQuestionReportAdminQuestionReportsReportIdPatchResponses[keyof UpdateQuestionReportAdminQuestionReportsReportIdPatchResponses];

export type IngestAnalyticsEventsAnalyticsEventsPostData = {
    body: IngestAnalyticsEventsBody;
    path?: never;
    query?: never;
    url: '/analytics/events';
};

export type IngestAnalyticsEventsAnalyticsEventsPostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type IngestAnalyticsEventsAnalyticsEventsPostError = IngestAnalyticsEventsAnalyticsEventsPostErrors[keyof IngestAnalyticsEventsAnalyticsEventsPostErrors];

export type IngestAnalyticsEventsAnalyticsEventsPostResponses = {
    /**
     * Successful Response
     */
    200: IngestAnalyticsEventsResponse;
};

export type IngestAnalyticsEventsAnalyticsEventsPostResponse = IngestAnalyticsEventsAnalyticsEventsPostResponses[keyof IngestAnalyticsEventsAnalyticsEventsPostResponses];

export type PublicSeasonsBillingSeasonsGetData = {
    body?: never;
    path?: never;
    query?: never;
    url: '/billing/seasons';
...
export type PublicSeasonsBillingSeasonsGetResponses = {
    /**
     * Successful Response
     */
...
export type PublicSeasonsBillingSeasonsGetResponse = PublicSeasonsBillingSeasonsGetResponses[keyof PublicSeasonsBillingSeasonsGetResponses];

export type MyBillingStatusBillingMeGetData = {
    body?: never;
    path?: never;
    query?: never;
    url: '/billing/me';
...
export type MyBillingStatusBillingMeGetResponses = {
    /**
     * Successful Response
     */
    200: BillingStatusResponse;
...
export type MyBillingStatusBillingMeGetResponse = MyBillingStatusBillingMeGetResponses[keyof MyBillingStatusBillingMeGetResponses];

export type CreateCheckoutSessionBillingCheckoutPostData = {
    /**
     * Body
     */
...
export type CreateCheckoutSessionBillingCheckoutPostErrors = {
    /**
     * Validation Error
     */
    422: HttpValidationError;
...
export type CreateCheckoutSessionBillingCheckoutPostError = CreateCheckoutSessionBillingCheckoutPostErrors[keyof CreateCheckoutSessionBillingCheckoutPostErrors];

export type CreateCheckoutSessionBillingCheckoutPostResponses = {
    /**
     * Successful Response
     */
...
export type CreateCheckoutSessionBillingCheckoutPostResponse = CreateCheckoutSessionBillingCheckoutPostResponses[keyof CreateCheckoutSessionBillingCheckoutPostResponses];
