...
export function useSignupMutation({ onSuccess, onError }: Props) {
    return useMutation({
        mutationFn: async (input: UserSignup & { country?: string }) => {
            // The generated client's UserSignup does not carry the
            // attribution fields or country yet — regenerating it rewrites
            // all ~1500 lines — so they are spread in and the body widened
            // here.
